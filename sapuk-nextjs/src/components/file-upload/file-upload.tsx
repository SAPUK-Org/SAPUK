...
export type FileUploadProps = UseFileUploadReturn & {
  accept?: string;
  maxSize?: number;
  maxFiles?: number;
  className?: string;
...
export type FileUploadInputProps<TUploadResult = void> =
...
export function FileUploadInput<TUploadResult = void>({
  ...
  disabled,
  ...uploadOptions
}: FileUploadInputProps<TUploadResult>) {
  const upload = useFileUpload<TUploadResult>(uploadOptions);
...
export function FileUpload({
  ...
  const limitsLabel = [
    maxSize ? `Files up to ${formatBytes(maxSize)}` : null,
  ...
  return (
    <motion.div
      layout
      role={!isEngaged && !disabled ? "button" : undefined}
      tabIndex={!isEngaged && !disabled ? 0 : undefined}
      aria-label={!isEngaged && !disabled ? "Upload files" : undefined}
      ...
      className={cn(
        ...
        "border-muted-foreground/25 bg-card shadow-sm",
        "transition-colors",
        !isEngaged && !disabled && [
          "cursor-pointer",
        ...
        drag && !disabled && "border-primary bg-primary/10",
        isEngaged ? "p-4" : "p-10",
        className,
      )}
    >
              ...
              >
                ...
                {canAddMore && (
                  <Button
                    ...
                    className="h-8 rounded-full border border-border bg-background px-3 text-xs shadow-sm hover:bg-muted"
                  >
                    Add more
                  </Button>
                )}
...
type FileUploadItemProps = {
  ...
  stateTransition: { duration: number; ease?: [number, number, number, number] };
    ...
    >
      <div
        className={cn(
          ...
          "hover:border-muted-foreground/30 hover:bg-muted/45",
      ...
      >
        <div className="flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-md border border-border bg-background">
          <AnimatePresence initial={false} mode="wait">
            {file.preview ? (
              <motion.img
                key="preview"
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm leading-5">{file.file.name}</p>
              ...
              {file.status === "uploading" ? (
                ...
                >
                  <div
                    ...
                    className="h-2 flex-1 overflow-hidden rounded-full bg-primary/15 ring-1 ring-primary/20"
                  >
                    <motion.div
                      initial={{ scaleX: 0 }}
                      animate={{
                ...
                <motion.p
                  ...
                  exit={{ opacity: 0 }}
                  transition={stateTransition}
                  className="flex h-5 items-center truncate text-[11px] font-medium leading-none text-destructive"
                >
                ...
                <motion.p
                  ...
                  exit={{ opacity: 0 }}
                  transition={stateTransition}
                  className="flex h-5 items-center text-[11px] font-medium leading-none text-emerald-700 dark:text-emerald-300"
                >
                  Uploaded and saved
                </motion.p>
              ) : (
        ...
        <div className="grid w-25 shrink-0 grid-cols-[1.75rem_2rem_2rem] items-center gap-1">
            ...
            <AnimatePresence initial={false} mode="wait">
              {file.status === "uploading" ? (
                ...
                <motion.span
                  ...
                  className="flex size-7 items-center justify-center text-emerald-700 dark:text-emerald-300"
                  aria-hidden="true"
                >
          ...
          <Button
            ...
            title="Retry upload"
            disabled={disabled || file.status !== "error"}
            ...
            className={cn(
              "size-8 rounded-full hover:bg-destructive/10 hover:text-destructive",
              file.status !== "error" && "pointer-events-none invisible",
            )}
          >
...
export type { FileState, UseFileUploadOptions, UseFileUploadReturn };
